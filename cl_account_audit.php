...
$myencrypted = $_GET['gtrfEEESskljH'];
$myencrypted = preg_replace('/\s+/', '+', $myencrypted);
$mydecrypted = rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5($mykey), base64_decode($myencrypted), MCRYPT_MODE_CBC, md5(md5($mykey))), "\0");
$clientntnumber = $mydecrypted;

$auditrecors = "SELECT client_account,client_firstname,client_lastname,mytimr,description,deposit,withdrawl,current_balance,dat_opn  FROM clients  JOIN account_audit ON clients.clinet_id = account_audit.cl_id  WHERE clinet_id = '$clientntnumber' order by dat_opn";
...
<div class="form-group">
 ...
 <input type="text" class="form-control" id="myuserid" name="myuserid" value="<?php echo $userid;?>" autocomplete="off" required style="display:none;" >
			 <input type="text" class="form-control" id="clientnumber" name="clientnumber" value="<?php echo $clientntnumber;?>" autocomplete="off" required  style="display:none;">	
	        ...
	        <div class="form-group">
	        	...
	        	<label class="col-sm-1 control-label">: </label>
				    <div class="col-sm-8">
				      <input type="text" name="datesearch" id="datesearch" autocomplete="off" required aria-required='true' aria-describedby='name-format'placeholder="Date of Operation" class="form-control datepicker">
				    </div>
	        </div> 
