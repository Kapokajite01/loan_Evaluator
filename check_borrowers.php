...
$borrowers = "SELECT first_name,last_name,id_number,telephone,district,sector,cell,village, loan_account,branch,loan_term_type,loan_term_number,loan_interest,first_payment,loan_type,loan_amount, MONTH(time_application) AS TM FROM loan_personalinfo INNER JOIN  loan_details ON loan_personalinfo.loan_id = loan_details.loan_tracknumber WHERE time_application >= '$from' and time_application <= '$to'";

}
else{
	$borrowers = "SELECT first_name,last_name,id_number,telephone,district,sector,cell,village, loan_account,branch,loan_term_type,loan_term_number,loan_interest,first_payment,loan_type,loan_amount, MONTH(time_application) AS TM FROM loan_personalinfo INNER JOIN  loan_details ON loan_personalinfo.loan_id = loan_details.loan_tracknumber WHERE MONTH(time_application) = MONTH(CURRENT_DATE())and YEAR(time_application) = YEAR(CURRENT_DATE())";
...
while($rowsborowers= $resultborrowers->fetch_assoc()) {
	...
	?>
<tr><td><?php echo $i;?></td>
<td><?php echo $rowsborowers['first_name'];?></td>
<td style='mso-number-format:"\@"'><?php echo $rowsborowers['last_name'];?></td>
<td><?php echo $rowsborowers['id_number'];?></td>
<td style='mso-number-format:"\@"'><?php echo $rowsborowers['telephone'];?></td>
<td style='mso-number-format:"\@"'><?php echo $rowsborowers['loan_account'];?></td>
<td><?php echo $rowsborowers['district'].'- '. $rowsborowers['sector'].'- '.$rowsborowers['cell'].'- '.$rowsborowers['village'];?></td>
<td><?php echo $rowsborowers['branch'];?></td>
...
<tr><td colspan="8"><h5 align="right"><strong>Total  Application Received </td><td colspan="4" style=" text-indent: 10%;"><h5><strong><?php echo number_format($totalamount);?></h5></strong></td></tr>
<?php }?> 
</table></form><hr>
