...
<?php 
$auditrecors = "SELECT DISTINCT Auditname FROM account_audit JOIN clients ON clients.clinet_id = account_audit.cl_id ";
?>
				</div>
			<?php 
...
while($rowaudit = $resultAUDIT->fetch_assoc()) {
	$AUDITNAME = $rowaudit['Auditname'];
?>
<tr>
<td><?php echo ++$i;?></td>
<td><?php echo  $AUDITNAME;?></td>
<td><?php 
$nmberof= "SELECT DISTINCT account_number FROM account_audit JOIN clients ON clients.clinet_id = account_audit.cl_id WHERE Auditname = '$AUDITNAME'";
$resultnmberof = $conn_db->query($nmberof);
if ($resultnmberof->num_rows > 0) {
$rowcount=mysqli_num_rows($resultnmberof);
echo $rowcount;
}


?></td>
<td><a href="aiditor_details?MYBRANCH=<?php echo  $AUDITNAME;?>"> Details</a></td>
