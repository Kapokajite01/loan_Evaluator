...
if(isset($_POST['datedisplay'])){
	...
	if($type == 'All'){
$borrowers = "SELECT first_name,last_name,id_number,telephone,district,sector,cell,village,loan_tracknumber, loan_account,loan_status,branch,loan_term_type,loan_term_number,loan_interest,time_application,first_payment,last_payment,loan_type,loan_amount, MONTH(time_application) AS TM FROM loan_personalinfo INNER JOIN  loan_details ON loan_personalinfo.loan_id = loan_details.loan_tracknumber WHERE (first_payment <= '$to' and last_payment >= '$from') AND loan_status ='Approved' ORDER  BY first_payment";
	}
else if($type != 'All'){
$borrowers = "SELECT first_name,last_name,id_number,telephone,district,sector,cell,village,loan_tracknumber, loan_account,loan_status,branch,loan_term_type,loan_term_number,loan_interest,time_application,first_payment,last_payment,loan_type,loan_amount, MONTH(time_application) AS TM FROM loan_personalinfo INNER JOIN  loan_details ON loan_personalinfo.loan_id = loan_details.loan_tracknumber WHERE (first_payment <= '$to' and last_payment >= '$from') and loan_type = '$type' AND loan_status ='Approved' ORDER  BY first_payment";
	
}
}
else{
  		$borrowers = "SELECT first_name,last_name,id_number,telephone,loan_tracknumber,district,sector,cell,village, loan_account,loan_status,branch,loan_term_type,loan_term_number,loan_interest,time_application,first_payment,last_payment,loan_type,loan_amount, MONTH(time_application) AS TM FROM loan_personalinfo INNER JOIN  loan_details ON loan_personalinfo.loan_id = loan_details.loan_tracknumber WHERE (first_payment <= '$lastDayThisMonth' AND last_payment >= '$firstDayThisMonth') AND loan_status ='Approved'   ORDER  BY first_payment";
...
<input type="text" name="datesearch" id="datesearch" placeholder="Start Date" autocomplete="off" required aria-required='true' aria-describedby='name-format'placeholder="From">
<input type="text" name="datesearch1" id="datesearch1" placeholder="End Date" autocomplete="off" required aria-required='true' aria-describedby='name-format'placeholder="To">
...
<th><strong>First payment</strong></th>
<th><strong>Term(s)</strong></th>
<th><strong>Last payment</strong></th>
<th><strong>Interest<br> Rate</strong></th>
<th><strong>Monthly<br> Payment</strong></th>
<tr>
<?php
...
while($rowsborowers= $resultborrowers->fetch_assoc()) {
	...
	$lastpymt = $rowsborowers['first_payment'];
	$pymtterm = $rowsborowers['loan_term_type'];
	$i++;
...
<td align="right"><?php echo number_format($rowsborowers['loan_amount'],2);?></td>
...
echo number_format($monthly_payment ,2);
?></td>
...
<tr><td colspan="5"><h5 align="right"><strong>Totals</td><td colspan="5"  align="left"><h5><strong><?php echo number_format($totalamount ,2);?></h5></strong></td><td align="right"><h5><strong><?php echo number_format($totamontlypaymet,2);?></h5></strong></td></tr>
<?php }?> 
</table></form><hr>
