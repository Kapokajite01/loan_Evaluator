...
$conn = mysqli_connect("localhost", "root", "fidele", "mudb_ims_db");
	$result = '';
	$query1 = "SELECT total_incidents FROM map WHERE district_name ='KARONGI' OR district_name ='NGORORERO' OR district_name ='NYABIHU' OR district_name ='NYAMASHEKE' OR district_name ='RUBAVU' OR district_name ='RUSIZI' OR district_name ='RUTSIRO'";
	$W;
	$mynum =0;
	$sql = mysqli_query($conn, $query1);
while($row = mysqli_fetch_array($sql))
		{
		$W[] = $row['total_incidents'];	
		}
$num = count($W);
				for($i=0;$i<=$num; $i++){
					if($W[$i] >0){
...
$num = count($f);
?>
<body style="background-color: #ffffff ; background-image:url(images/mapdisplay_western.png); background-repeat: no-repeat;background-position:center;">

	<div class="lg-container">
		<?php 
echo "<strong>"."Actor group Selected:"."</strong>"."<font color=red>".$_SESSION['actors_group_name']."</font>";
echo "<br>";
...
echo '<img src="images/elsepin.png"></img>';
					...
					if($f[7] !=0){
echo '<img src="images/mappinpeople.png"></img>';
						echo '<font color="black"><strong>'. $f[7];
					...
					if($f[8] !=0){
echo '<img src="images/mappinpeople.png"></img>';
						echo '<font color="black"><strong>'.$f[8];
