...
<form action="top_incidents1" method="POST">
...
<br>
<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="top_incidents"><button class="btn btn-default button1"> <B>
<font color="black">Top Incidents</B></font></button></a>
<br>
<br>
...
count(incident_name) FROM reports WHERE ((dateincident BETWEEN '$startdate' AND '$enddate')and YEAR(dateincident)= YEAR(CURDATE()))  GROUP BY  incident_name";
if ($conn->multi_query($sql) === TRUE) {
...
foreach ($result as $row) {
	$incident[] = $row['keywords2'];
	$total[] = $row['total_incidents'];
}
	$numdt = count($incident);
for($i=0;$i<$numdt;$i++){
}
mysqli_close($conn);
    ...
    $dataPoints1 = array(
    	array("label"=> $incident[0], "y"=>  $total[0]),
    	array("label"=> $incident[1], "y"=>  $total[1]),
    	array("label"=> $incident[2], "y"=>  $total[2]),
    	array("label"=> $incident[3], "y"=>  $total[3]),
    	array("label"=> $incident[4], "y"=>  $total[4]),
    	array("label"=> $incident[5], "y"=>  $total[5]),
    	array("label"=> $incident[6], "y"=>  $total[6]),
    	array("label"=> $incident[7], "y"=>  $total[7]),
    	array("label"=> $incident[8], "y"=>  $total[8]),
    	array("label"=> $incident[9], "y"=>  $total[9])
    );
    ...
    var chart = new CanvasJS.Chart("chartContainer", {
    	...
    	theme: "light2",
    	title:{
    		text: "Top  Incidents From <?php  echo $newDate;?> To <?php  echo $newDate1;?> "
    	},
    	legend:{
    		cursor: "pointer",
