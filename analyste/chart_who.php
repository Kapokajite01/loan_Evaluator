...
<form action="chart_who1" method="POST">

 <?php
mysql_connect("localhost", "root","fidele") or die(mysql_error());
mysql_select_db("mudb_ims_db") or die(mysql_error());

$query = "SELECT DISTINCT actors_in_group_name FROM actors_in_groups ORDER BY actors_in_group_name ASC";
$result = mysql_query($query) or die(mysql_error()."[".$query."]");
?>

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<select name="actor_name" style="max-width:90%;" required><option value="">Select Actor Group</option>
<?php 
...
$MYTIME = "ACTING GROUPS IN".date(' F Y ');



$sql = "INSERT INTO chart (district_name, keywords,keywords2,keywords3,district_code,label, total_incidents) 
SELECT district_name, actor_name,incident_name,dateincident,district,CONCAT(district_name,neighbor_country),
count(actor_name) FROM reports WHERE  MONTH(dateincident)= MONTH(CURDATE())and YEAR(dateincident)= YEAR(CURDATE())  GROUP BY actor_name";
...
mysql_connect("localhost", "root","fidele") or die(mysql_error());
mysql_select_db("mudb_ims_db") or die(mysql_error());
$query = "SELECT id,district_name,keywords,keywords2,keywords3,district_code,label,total_incidents FROM chart order by total_incidents DESC";
$result = mysql_query($query) or die(mysql_error()."[".$query."]");
while ($row = mysql_fetch_array($result))
{
	$incid[] = $row['keywords'];
