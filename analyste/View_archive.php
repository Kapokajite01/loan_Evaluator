...
$status=$_GET['Status'];
    ...
    $query="";
    if($status=="Archive_Deleted"){
        $query = "SELECT * FROM report_archive WHERE Archive_report_id ='$id_number'";
    }else{
        $query = "SELECT * FROM report_archive WHERE Archive_report_id ='$id_number'";
    }
...
$num =0;
	...
	<br><br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" onClick="return confirm('Are you sure you want to Restore this report?')" name="Restore" value="RESTORE" id="bigbutton"/>
			<?php	}else{
                echo'';
            }
                        ?>
               ...
               if(isset($_POST['Restore']))
               {
                ...
                $Restore=mysql_query("INSERT INTO reports(province,district,sector,cell,latitude,longitude,geodescription,dateincident,dateentry,incident,incident_name,keyword,keyword_name,subkeword,message,actorgroup,actorgoup_name,Institution,institution_name,actor,actor_name,memebers,member_name,actor_desrciption,memberdescription,classification,classification_name,credibility,credidbility_name,reliabaility,reliability_name,source,source_name,povince_name,district_name,sector_name,cell_name,subject,noattachments,directorate,neighbor_country,neighbor_province,neighbor_district,user_id) select province,district,sector,cell,latitude,longitude,geodescription,dateincident,dateentry,incident,incident_name,keyword,keyword_name,subkeword,message,actorgroup,actorgoup_name,Institution,institution_name,actor,actor_name,memebers,member_name,actor_desrciption,memberdescription,classification,classification_name,credibility,credidbility_name,reliabaility,reliability_name,source,source_name,povince_name,district_name,sector_name,cell_name,subject,noattachments,directorate,neighbor_country,neighbor_province,neighbor_district,user_id from report_archive where Archive_report_id='".$repoid."'") ;
                
                
                if($Restore)
                {
                   
                    $del=mysql_query("DELETE FROM `mudb_ims_db`.`report_archive` WHERE `report_archive`.`Archive_report_id` = '".$repoid."'");
                    
                    if($del)
                    {
                        $up2=mysql_query("UPDATE `mudb_ims_db`.`report_archive` SET `Status`='Archive_Edited' WHERE `report_archive`.`Archive_report_id` = '".$repoid."'");
                        if($up2){
                    echo "<script>alert('Report Retored Successfully!!!')</script>";
                        echo "<script>window.location='modified';</script>";
                        }else{
