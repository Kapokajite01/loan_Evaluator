...
$borrowers = "SELECT first_name,last_name,id_number,telephone,district,sector,cell,village,loan_tracknumber, loan_account,loan_status,branch,loan_term_type,loan_term_number,loan_interest,time_application,first_payment,loan_type,loan_amount, MONTH(time_application) AS TM FROM loan_personalinfo INNER JOIN  loan_details ON loan_personalinfo.loan_id = loan_details.loan_tracknumber WHERE loan_type ='$NTYPE' AND time_application >= '$from' and time_application <= '$to'  ORDER  BY time_application ";
	
}
else{
	$borrowers = "SELECT first_name,last_name,id_number,telephone,loan_tracknumber,district,sector,cell,village, loan_account,loan_status,branch,loan_term_type,loan_term_number,loan_interest,time_application,first_payment,loan_type,loan_amount, MONTH(time_application) AS TM FROM loan_personalinfo INNER JOIN  loan_details ON loan_personalinfo.loan_id = loan_details.loan_tracknumber WHERE loan_type ='$NTYPE' AND MONTH(time_application) = MONTH(CURRENT_DATE()) and YEAR(time_application) = YEAR(CURRENT_DATE())  ORDER  BY time_application";
...
<br><br>
<h1>Loan Type : <?php echo $NTYPE;?></h1><hr>
...
while($rowsborowers= $resultborrowers->fetch_assoc()) {
	$i++;
 $Firtstname = $rowsborowers['first_name'];
 $lastbname = $rowsborowers['last_name'];
 $branche = $rowsborowers['first_payment'];
 $DT = $rowsborowers['first_payment'];
 $nmbr = $rowsborowers['loan_term_number'];
 $tp = $rowsborowers['loan_term_type'];	
	?>
<tr><td><?php echo $i;?></td>
<td><?php echo $Firtstname.'&nbsp;'.$lastbname;?></td>
...
<td align="right"><?php echo number_format($rowsborowers['loan_amount']);?></td>
 ...
 <td  align="center"><?php  echo $nmbr.'&nbsp;'.$tp;?>(s)</td>
...
?></td>
  ...
  <?php 
  $totnumloans = $totnumloans +$rowsborowers['numloans'];
  $totalamount = $totalamount + $rowsborowers['loan_amount'];
$totamontlypaymet = $totamontlypaymet + $monthly_payment;
  }?>
<tr><td colspan="3" ><h5 align="right"><strong>Totals</td><td align="right"><h5><strong><?php echo number_format($totalamount);?></h5></strong></td><td colspan="3" align="right"><h5><strong><?php echo number_format($totamontlypaymet);?></h5></strong></td></tr>
<?php }?> 
</table></form><hr>
