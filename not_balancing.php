...
$auditrecors = "SELECT clinet_id,client_account,client_firstname,client_lastname,cl_account_type,account_atatus,last_audit_balance,Auditname,variance,SUM(deposit) AS totdeposits,SUM(withdrawl) AS totwithdrawala,current_balance  FROM clients  JOIN account_audit ON clients.clinet_id = account_audit.cl_id WHERE  Auditname = '$branch' GROUP BY client_account";
	
}
else{
$auditrecors = "SELECT clinet_id,client_account,client_firstname,client_lastname,cl_account_type,account_atatus,last_audit_balance,Auditname,variance,SUM(deposit) AS totdeposits,SUM(withdrawl) AS totwithdrawala,current_balance  FROM clients  JOIN account_audit ON clients.clinet_id = account_audit.cl_id GROUP BY client_account";
}
?>
				</div>
			<?php 
